...
def init_weight_goog(
    ...
    * https://github.com/tensorflow/tpu/blob/master/models/official/efficientnet/efficientnet_model.py
    """
    if isinstance(ly, CondConv2D):
        fan_out = ly.kernel_size[0] * ly.kernel_size[1] * ly.out_channels
        if fix_group_fanout:
            fan_out //= ly.groups
        init_weight_fn = get_condconv_initializer(
            Normal(0, math.sqrt(2.0 / fan_out)), 
            ly.num_experts, ly.weight_shape)
        ly.weight = init_weight_fn(ly.weight)
        if ly.bias is not None:
            ly.bias = create_parameter(
                ly.bias.shape, 
                ly.bias.dtype, 
                ly.bias.name, 
                Constant(), 
                True)
    elif isinstance(ly, Conv2D):
        fan_out = ly._kernel_size[0] * ly._kernel_size[1] * ly._out_channels
        if fix_group_fanout:
            fan_out //= ly._groups
        ly.weight = create_parameter(
                ly.weight.shape, 
                ly.weight.dtype, 
                ly.weight.name, 
                Normal(0, math.sqrt(2.0 / fan_out)))
        ...
        fan_out = ly.weight.shape[0]  # fan-out
        fan_in = 0
        if 'routing_fn' in n:
            fan_in = ly.weight.shape[1]
